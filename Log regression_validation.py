# -*- coding: utf-8 -*-
"""
Created on Sat Sep  3 14:15:12 2022
...
results=abs(cross_val_score(LR,X,Y,cv=loocv, scoring="neg_mean_squared_error"))
results
np.mean(results)
...
KF=KFold(n_splits=506)
LR=LinearRegression()

results=abs(cross_val_score(LR,X,Y,cv=KF, scoring="neg_mean_squared_error"))
results
np.mean(results)
